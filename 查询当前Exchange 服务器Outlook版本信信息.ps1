#查询当前Exchange 服务器 Outlook 版本信信息
...
#Modules are required otherwise information is not retrieveable
        ...
        {
         Add-PSSnapin Microsoft.Exchange.Management.PowerShell.E2013 -EA SilentlyContinue
   }
        Else
...
-or (($_.IsMailboxServer -eq '$true') -and ($_.Site -eq 'hotfixtools.com/Configuration/Sites/shenzhen')) } | `
Select Name,@{Name='Path';Expression={("$($_.Datapath)").Replace("Mailbox","Logging\RPC Client Access")}} )



ForEach ($Item in $Servers)
{
$Thefile = @(GCI -Path $Item.Path -Filter *.log | ?{$_.LastWriteTime -gt (Get-Date).AddDays(-7)} | Select @{Name='File';Expression={("$($Item.Path)" + "\$($_.Name)")}})
